---
title:  "[Project] thread 적용으로 실시간 데이터 INSERT 및 SELECT "
...
int pthread_create(pthread_t *restrict thread,
                    ...
                    void *restrict arg);
```
- 첫번째 매개변수로 성공적으로 함수가 호출되면 이곳에 <span style="color:#00FF00"> thread ID </span> 가 저장된다. 이 인자로 넘어온 값을 통해서 <span style="color:yellow"> pthread_join() </span> 과 같은 함수를 응용 할 수 있다.
- 두번째 매개변수는 스레드의 특성을 정의합니다. 기본적으로 <span style="color:violet"> NULL </span> 을 지정한다. 아직은 잘 모르겠다.. <span style="color:#00FF00"> Multi-Thread </span> 를 배울 때 질문해보자..
- 세번째 매개변수는 어떤 로직을 할지 함수 포인터를 매개변수로 받는다. 
- 네번째 매개변수는 전달될 인자를 말한다. <span style="color:yellow"> start_routine() </span> 에서 이 인자를 변환하여 사용할 수 있다.
...
```
- 기본적인 사용법은 의외로 간단하다.<br>
반복되는 while문 안에서 <span style="color:yellow"> sleep() </span> 으로 시간의 텀을 주면된다.
...
- <span style="color:orange"> 원인: </span>DB에 관련된 함수들을 동시에 쓰는 것과 비슷하게 너무 짧은 간격으로 사용했기때문이라고 생각했다. 
- <span style="color:#00FF00"> 해결:  </span><span style="color:yellow"> sleep() </span>를 사용하여 시간의 텀을 주었더니 해결되었다 !
...
# 전 코드와의 변경점
```
1. log_cnt 라는 변수 추가 ( tb_packet_log 개수 파악 ) .
2. cmp_ret 변수를 allow 일 때 1로 나타내서 result 구분하게 함 ( 0은 block, 1은 allow ) .
3. 유해사이트 차단 목록을 저장하던 배열을 전역변수로 빼서 한 번 정의하고 재활용함 block_domain_arr .
4. get_update_status를 통해 3번째 쓰레드로 업데이트 했을 때 block_domain_arr 배열을 최신화함 .
5. block_domain_count를 전역변수로 선언해서 지역이 다른 select와 compare 부분을 해결함 .
6. 쓰레드를 위한 라이브러리와 변수 선언 .
7. 오류 처리를 위한 LINE 처리 .
```